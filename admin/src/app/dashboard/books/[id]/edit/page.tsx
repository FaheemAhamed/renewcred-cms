...
import { fetchBookById, updateBook } from '@/store/slices/bookSlice';
import { ArrowLeft, Save, FileText, Image as ImageIcon, Loader2 } from 'lucide-react';
...
export default function EditBookPage() {
  ...
  const { currentBook, isLoading, isSubmitting } = useAppSelector((state) => state.books);
  ...
  useEffect(() => {
    if (currentBook) {
      reset({
        ...
        discountPrice: currentBook.discountPrice || 0,
        status: currentBook.status,
        isFeatured: currentBook.isFeatured || false,
      });
    }
  }, [currentBook, reset]);
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Edit Book Details</h2>
          <p className="text-xs text-slate-500 mt-0.5">Editing &quot;{currentBook.title}&quot;</p>
        </div>
      </div>
      ...
      >
        ...
        {/* Current Media Files & New Uploads */}
        <div className="pt-4 border-t border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Cover Image Upload */}
          <div className="rounded-xl border border-slate-200 p-4 bg-slate-50/50 space-y-3">
            <span className="block text-xs font-bold text-slate-700">Cover Artwork</span>
            {currentBook.coverImage && (
              <div className="flex items-center gap-3">
                <div className="relative h-14 w-10 overflow-hidden rounded border border-slate-200">
                  <Image
                ...
                <span className="text-xs text-slate-500">Current cover image uploaded</span>
              </div>
            )}
            <input
              type="file"
          ...
          <div className="rounded-xl border border-slate-200 p-4 bg-slate-50/50 space-y-3">
            ...
            {currentBook.pdfUrl && (
              <div className="flex items-center gap-2 text-xs text-blue-600 font-semibold">
                ...
                <a href={currentBook.pdfUrl} target="_blank" rel="noreferrer" className="underline">
